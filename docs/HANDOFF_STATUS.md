...
- [~] Fleet governance expansion:
  ...
  - [~] fleet approval request/review + approved-execution matching is wired in-app and surfaced in dashboard scaffold (including approve/deny actions); scaffold API now enforces duplicate-pending + self-approval guardrails, pending-expiry normalization, and approved-execution claim tokens/metadata, while production orchestration rollout remains pending
