...
type NovaDeviceFallbackPanelProps = {
  serverId: string;
  sessions: string[];
  isPro: boolean;
...
function AgentCard({
  ...
  const contextSummary = spineContext
    ? `${spineStatusLabel(spineContext.status)} • events ${spineContext.totalEntries}${
        spineContext.pendingApprovalCount > 0 ? ` • pending ${spineContext.pendingApprovalCount}` : ""
      }`
    : "IDLE • events 0";
...
export function NovaDeviceFallbackPanel({
  serverId,
  sessions,
  isPro,
  surface,
  ...
  return (
    <>
      <Text style={styles.serverSubtitle}>
        {surface === "screen"
          ? "These controls run on this phone until the server runtime is available again."
          : "Preview-only device runtime controls."}
      </Text>
      <TextInput
        style={styles.input}
        value={newAgentName}
