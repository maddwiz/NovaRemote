...
describe("useVrWorkspace", () => {
  ...
  it("routes voice commands to the target server/session", async () => {
    ...
    const createAgentWithForPhraseAction = current().applyVoiceTranscript("create agent watch for errors");
    expect(createAgentWithForPhraseAction).toEqual({
      kind: "create_agent",
      serverIds: ["dgx"],
    ...
    const setAgentGoalWithForPhraseAction = current().applyVoiceTranscript(
      "set agent deploy bot goal run deploy for staging"
    );
    expect(setAgentGoalWithForPhraseAction).toEqual({
      kind: "set_agent_goal",
      serverIds: ["dgx"],
      name: "deploy bot",
    ...
    const approveHomeAction = current().applyVoiceTranscript("approve ready agents for homelab");
    expect(approveHomeAction).toEqual({
      kind: "approve_ready_agents",
      serverIds: ["home"],
    });
